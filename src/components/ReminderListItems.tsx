...
export default function ReminderListItem({ reminderItem }:ReminderListItemProps) {
    ...
    const { mutate: completedReminder } = useMutation({
        mutationFn: (completed: boolean) => completeReminder(reminderItem.id, completed),
        onSuccess: (data) => {
  ...
  return (
    ...
    {isCompleted
      ...
      <View style={{ gap: 5, flexShrink: 1 ,alignItems: 'flex-start',  display:'flex',flexDirection:"column"  }}>
        <Text>{reminderItem.reminder}</Text>
        {!!reminderItem.note && (
          <Text style={{ fontSize: 12, color: 'grey' }}>{reminderItem.note}</Text>
        )}
      </View>
      <AntDesign
        name="info-circle"
